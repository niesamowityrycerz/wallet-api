  ...
  class RankingPoint
    ...
    def call(event)
      if settlement_terms_added?(event)
        # send warning in x minutes
        binding.pry
        command_bus.call(Warnings::PrepareToSendTransactionExpiredWarning.perform_in(10.seconds, event.data.fetch(:transaction_uid)))        
      elsif transaction_settled?(event)
        #command_bus.call(Warnings::Commands::CancelTransactionExpiredWarning.)
