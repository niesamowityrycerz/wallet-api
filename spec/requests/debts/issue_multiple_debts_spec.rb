...
RSpec.describe 'Issue multipe debets at once action', type: :request do 
  ...
  context 'when access permitted' do 
    context 'when credit equally among members' do 
      it 'issues debts' do
        post "/api/v1/groups/#{group_uid}/add_debt", params: @params, headers: { 'Authorization': 'Bearer ' + access_token.token }

        expect(response.status).to eq(201)
        to_repay_per_debtor = (@params[:amount] / @params[:debtors_ids].count).round(2)
    
        expect(event_store).to have_published(
          an_event(Debts::Events::DebtIssued).with_data(amount: to_repay_per_debtor, debtor_id: leader_friends[0].id, creditor_id: leader.id),
          an_event(Debts::Events::DebtIssued).with_data(amount: to_repay_per_debtor, debtor_id: leader_friends[1].id, creditor_id: leader.id),
          an_event(Debts::Events::DebtIssued).with_data(amount: to_repay_per_debtor, debtor_id: leader_friends[2].id, creditor_id: leader.id)
